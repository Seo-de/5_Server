...
<head>
    ...
    <style>
        ...
        .last{background-color: black; color: white}
    </style>
</head>
<body>
    <h1>JSTL(Jsp Standard Tag Library) - JSP 표준 태그 모음</h1>

    <pre>
        JSP에서 자주 사용되거나 공통적으로 사용되는
        Java 코드의 표기법을 간단히 태그화하여 표준으로 제공하는 것.

        (if, for, 배열/컬렉션 길이, 문자열 치환, 숫자 데이터 형식 변경,
         데이터 파싱-데이터 형태 바꾸기, scope 변수 선언 등)
    </pre>
    ...
    <ol>
        ...
        <li>/webapp/WEB-INF/lib에 추가하기</li>
    </ol>
   ...
   <% pageContext.setAttribute("num1", 10); %>
    ...
    <%-- JSTL 작성법 --%> <%-- HTML 코드처럼 작성하는 Java 코드 --%>
    ...
    <h4>${num2 }</h4> <!-- 20 -> 우선순위가 달라서. page가 먼저잖아 -->
	...
	<c:choose>
		<c:when test="${ temp2 > 100 }"> 100 보다 크다</c:when>
	
		<c:when test="${ temp2 < 100 }"> 100 보다 작다</c:when>
	...
	<pre>
		...
		6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수

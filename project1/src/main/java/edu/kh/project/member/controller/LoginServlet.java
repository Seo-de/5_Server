...
//			req.setAttribute("loginMember", loginMember);
			...
			if(loginMember != null) { // 로그인 성공 시
				...
				session.setMaxInactiveInterval(60*60); // 지정 안하면 무제한임. 60초 * 60분 == 1시간(3600초)
				...
				// 2) 아이디 저장 체크 박스가 체크 되었는지 확인 (체크:on / 체크x:null)
				if(req.getParameter("saveId") != null ) { // 체크 된 경우
					...
					cookie.setMaxAge(0); // 저장되지 않는거지
				}
				
				// 5) 생성된 쿠키가 적용되어질 요청 주소 지정
